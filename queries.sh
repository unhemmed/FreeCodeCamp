...
echo "$($PSQL "SELECT DISTINCT(t.name) FROM teams AS t RIGHT JOIN games AS g1 ON t.team_id=g1.winner_id ORDER BY t.name;")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT g1.year,t.name FROM teams AS t RIGHT JOIN games AS g1 ON t.team_id=g1.winner_id WHERE g1.round='Final' ORDER BY year;")"
